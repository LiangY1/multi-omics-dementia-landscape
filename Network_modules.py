...
#target = 'AD'
#target = 'VaD'
dict_df = pd.read_csv('../OmicsNetwork/FeatureDict.csv')
mydf = pd.read_csv('../OmicsNetwork/'+target+'_SigAss.csv')
...
gdf = pd.merge(gdf, dict_df, how = 'left', on = 'OmicFeature')

gdf.to_csv('../OmicsNetwork/Louvain_'+target+'.csv')
